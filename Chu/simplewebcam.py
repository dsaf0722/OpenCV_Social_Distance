...
def Chu2():
    cv2.nameWindow = ("head_detection",cv2.WINDOW_NORMAL)
    # use pretrain model to detect head
    ...
    def crop_face(image, bbox, wrap=True):
        ...
        if x1 < 0 or y1 < 0 or x2 > width or y2 > height:
            ...
            vw = vx2 - vx1
            vh = vy2 - vy1
            patch[sy:sy+vh, sx:sx+vw] = image[vy1:vy2, vx1:vx2]
            return patch
        return image[y1:y2, x1:x2]

    def mtcnn_detection(image, scales, width, height):
        ### pnet ###
        bboxes_in_all_scales = np.zeros((0, 4 + 1 + 4), dtype=np.float32)
        for scale in scales:
            w, h = int(math.ceil(scale * width)), int(math.ceil(scale * height))
            data = cv2.resize(image, (w, h))
    ...
    while(True):
        ...
        ret, image = cap.read()
        boxes_list = []
        onet_boxes = mtcnn_detection(image, scales, width, height)
        imgdraw_onet = image.copy() 
                ...
                for j in range(boxes):
                    for n in range(boxes):
                        if j != n and n > j:
                            ...
                            # list_[j][7] = box_center_y
                            v_distance = np.abs(list_[j][7]-list_[n][7])
                            v_distance_to_others.append([boxes,j,n,v_distance])
                            ...
                            real_distance_of_objects_ = round(np.abs((list_[n][-1])**2 + (list_[j][-1])**2 - (2*(list_[n][-1])*(list_[j][-1]) * math.cos(A_)))**0.5,2)
    
                            # to consider z-axis final_distance = (real_distance_of_objects**2 + v_distance_to_others**2)**0.5
                            real_distance_of_objects = (real_distance_of_objects_**2 + v_distance**2)**0.5
...
#                                         cv2.putText(imgdraw_onet,f'person {result[2]}',(list_[result[2]][0],list_[result[2]][1]-10), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
    ...
    #         cv2.putText(imgdraw_onet, '%.03f'%score, (x1, y1), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
        ...
        imgdraw_onet2 = imutils.resize(imgdraw_onet,width=200,height=200)
        cv2.imshow("Social_Disatnce", imgdraw_onet2)
        image = cv2.imencode('.jpg', imgdraw_onet)[1].tobytes()
